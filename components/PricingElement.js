            ...
            <div className="m-auto border-2 border-t-0 border-gray-200 w-11/12 text-align flex flex-col justify-center">
                {
                    content.map(function(text, index){
                        return (
                            text.active ? (<p key={index} className="mt-4 mb-4 lg:text-sm text-xs flex text-center w-full justify-center items-center"><span className="ml-5">{getIcons('check')}</span> <span className="ml-5">{text.title}</span></p>)
